...
# Set JELLYBAM_BUILDTYPE
ifdef JELLYBAM_NIGHTLY
    JELLYBAM_BUILDTYPE := NIGHTLY
endif
ifdef _EXPERIMENTAL
    JELLYBAM_BUILDTYPE := EXPERIMENTAL
endif
ifdef CM_RELEASE
    JELLYBAM_BUILDTYPE := RELEASE
endif

ifdef CM_BUILDTYPE
    ifdef JELLYBAM_EXTRAVERSION
        # Force build type to EXPERIMENTAL
        JELLYBAM_BUILDTYPE := EXPERIMENTAL
        # Add leading dash to CM_EXTRAVERSION
        JELLYBAM_EXTRAVERSION := -$(CM_EXTRAVERSION)
    endif
else
    # If CM_BUILDTYPE is not defined, set to UNOFFICIAL
    JELLYBAM_BUILDTYPE := STABLE
    JELLYBAM_EXTRAVERSION :=
endif

ifdef JELLYBAM_RELEASE
    JELLYBAM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(JELLYBAM_BUILD)
else
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        JELLYBAM_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d)-$(JELLYBAM_BUILDTYPE)-$(JELLYBAM_BUILD)$(JELLYBAM_EXTRAVERSION)
    else
        JELLYBAM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(JELLYBAM_BUILDTYPE)-$(JELLYBAM_BUILD)$(JELLYBAM_EXTRAVERSION)
    endif
endif
