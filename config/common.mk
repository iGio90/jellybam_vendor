...
ifdef JELLYBAM_BUILDTYPE
    ifdef JELLYBAM_EXTRAVERSION
        # Force build type to EXPERIMENTAL
        JELLYBAM_BUILDTYPE := EXPERIMENTAL
        # Add leading dash to JELLYBAM_EXTRAVERSION
        JELLYBAM_EXTRAVERSION := -$(JELLYBAM_EXTRAVERSION)
    endif
else
    # If CM_BUILDTYPE is not defined, set to UNOFFICIAL
    JELLYBAM_BUILDTYPE := STABLE
    JELLYBAM_EXTRAVERSION := STABLE
endif
...
ifdef JELLYBAM_RELEASE
    ...
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        JELLYBAM_VERSION := v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-$(JELLYBAM_BUILDTYPE)-$(JELLYBAM_BUILD)$(JELLYBAM_EXTRAVERSION)
    else
        JELLYBAM_VERSION := v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-$(JELLYBAM_BUILDTYPE)-$(JELLYBAM_BUILD)$(JELLYBAM_EXTRAVERSION)
    endif
endif
